...
public:
	...
	void check_avl_tree(Node* point);
	void rotation_right(Node* point);
	void rotation_left(Node* point);
...
template <typename T>
AVL<T>::Node::Node(T element) : element(element), leftChild(nullptr), rightChild(nullptr)
{
}
...
template <typename T>
void AVL<T>::rec_find_preorder(std::vector<T>& vec, Node* sub_tree)
{
...
void AVL<T>::remove(T element)
{
	if(find(element))
	{
		...
		if (remove_branch->leftChild != nullptr)
		{
			...
			check_avl_tree(remove_branch->parent);
		}
...
template <typename T>
void AVL<T>::check_avl_tree(Node* point)
{
	if (point != nullptr)
	{
		if (point->leftChild != nullptr or point->rightChild != nullptr)
		{
			if (rec_treeheight(point->leftChild) - rec_treeheight(point->rightChild) >= 2) //Heavy left side
			{
				if (rec_treeheight(point->leftChild->rightChild) < rec_treeheight(point->leftChild->leftChild))
				{
					rotation_right(point->leftChild);
				}
				rotation_right(point);
			}

			else if (rec_treeheight(point->rightChild) - rec_treeheight(point->leftChild) >= 2) //Heavy right side
			{
				if (rec_treeheight(point->rightChild->leftChild) < rec_treeheight(point->rightChild->rightChild))
				{
					rotation_left(point->rightChild);
				}
				rotation_left(point);
			}
		...
		{
			check_avl_tree(point->parent);
		}
	}
}

template <typename T>
void AVL<T>::rotation_right(Node* point)
{
	Node* new_parent = point->leftChild;
	Node* ganny_parent = point->parent;
	Node* swaper = point->leftChild->rightChild;
	if (point == root)
	{
		root = new_parent;
		new_parent->parent = nullptr;
	}
	else if (ganny_parent->rightChild == point)
	{
		ganny_parent->rightChild = new_parent;
		new_parent->parent = ganny_parent;
	}
	else
	{
		ganny_parent->leftChild = new_parent;
		new_parent->leftChild = ganny_parent;
	}
	swaper = new_parent->rightChild;
	new_parent->rightChild = point;
	if (swaper != nullptr)
	{
		swaper->parent = point;
	}
	point->parent = new_parent;
	point->leftChild = swaper;
}

template <typename T>
void AVL<T>::rotation_left(Node* point)
{
	Node* new_parent = point->rightChild;
	Node* ganny_parent = point->parent;
	Node* swaper = point->rightChild->leftChild;
	if (point == root)
	{
		root = new_parent;
		new_parent->parent = nullptr;
	}
	else if (ganny_parent->leftChild == point)
	{
		ganny_parent->leftChild = new_parent;
		new_parent->parent = ganny_parent;
	}
	...
	{
		ganny_parent->rightChild = new_parent;
		new_parent->rightChild = ganny_parent;
	}
	swaper = new_parent->leftChild;
	new_parent->leftChild = point;
	if (swaper != nullptr)
	{
		swaper->parent = point;
	}
	point->parent = new_parent;
	point->rightChild = swaper;
}
...
template <typename T>
void AVL<T>::rec_find_inorder(std::vector<T>& vec, Node* sub_tree)
{
...
template <typename T>
void AVL<T>::rec_find_postorder(std::vector<T>& vec, Node* sub_tree)
{
	if (sub_tree->leftChild != nullptr)
	{
...
template <typename T>
int AVL<T>::rec_treeheight(Node* current)
{
	if (current == nullptr)
	{
		return 0;
