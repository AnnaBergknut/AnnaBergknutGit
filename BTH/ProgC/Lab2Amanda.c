...
{
    if( yVal > CAM_HEIGHT-(CAM_HEIGHT/3))
    {
        printf("Back \n");
    }
...
int ProcessImage(const unsigned char *_yuv, int _size, void *pixels)
{
    ...
    #pragma omp parallel num_threads(threads)
    {
        ...
        for (int i = 4*id; i < _size; i += threads*4)
        {
            ...
            rgbIndex += (threads*2)-1;
            if (found == 1)
            {
                int j = i/2;
                printf("found \n");
                int xVal = j%CAM_WIDTH;
                int yVal = floor(j/CAM_HEIGHT);
...
int main()
{
    ...
    while (1)
    {
        ...
        ProcessImage(imageMemory[bufs[idx].index], bufs[idx].bytesused, pixels);
